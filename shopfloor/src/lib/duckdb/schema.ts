...
export function persistExportPath(table: TableName): string {
  return `persist-${table}.parquet`
}

export function insertParquetByName(table: TableName, path: string): string {
  const cols = TABLE_COLUMNS[table].join(", ")
  return `INSERT INTO ${table} BY NAME SELECT ${cols} FROM read_parquet('${path}')`
}
